...
from data.STRINGS import MENU_REF_BUTTON, MENU_REF_TEXT, MENU_CABINET_BUTTON, MENU_CABINET_TEXT, \
    ...
    CALLBACK_CHANNEL_ARROVED, CALLBACK_CHANNEL_DISAPPROVED, ADMINS, LINK_USER, CHANNEL
...
async def withdraw(query: types.CallbackQuery):
    ...
    await query.bot.send_message(CHANNEL[0], "Ishladi!", reply_markup=inline_channel)
...
async def withdraw(query: types.CallbackQuery):
    if query.from_user.id in ADMINS:
        ...
        await query.bot.send_message(CHANNEL[0],
                                     f'<a href="{LINK_USER}{users_id[-1]}">{users_id[-1]}</a> ga Uc to\'landi!')
        try:
            await query.bot.send_message(users_id[-1], "Uc to'landi!")
            users_id.clear()
        except Exception:
...
async def withdraw(query: types.CallbackQuery):
    if query.from_user.id in ADMINS:
        await query.bot.delete_message(CHANNEL[0], query.message.message_id)
        await query.bot.send_message(query.from_user.id, "Uc to'lash rad etildi!")
    else:
        await query.answer(text="Bu tugma siz uchun emas!", show_alert=True)
